...
rw = w.add_run("Field Manager")
rw.font.bold = True
w.add_run(
    " — the boots on the ground and the organizational backbone of how this company runs hotels. You will work directly "
    "with me, day to day, keeping every property we serve audited, running, and ahead of its problems."
)
...
duties = [
    ("Daily audit", "every active property, every morning — occupancy, arrivals and departures, incidents, work orders, and open follow-ups — walked, checked, and reconciled before the day gets away from anyone."),
    ("On the ground", "physically present at the properties — walking rooms, verifying work orders, checking crews, and being the eyes I can't be at every building every day."),
    ("Calendar & maintenance execution", "the preventive plan per property, executed — inspections, maintenance visits, and deadlines scheduled, tracked, and chased to completion."),
    ("Coordination between teams", "the connective tissue between front desk, housekeeping, maintenance, and ownership — so nothing falls in the gap between departments."),
...
r3 = cp2.add_run("A field retainer that grows with the portfolio — every hotel Attenda onboards adds $600 to your monthly retainer.")
...
rq = qp.add_run("\u201cYou are not joining a company to watch properties from a desk. You are joining to walk them with me \u2014 "
                "hotels only run calmly when somebody is standing on the floor, seeing what needs seeing before anyone has to ask. That somebody is you.\u201d")
...
out = "/Users/thrilzco/Documents/Attenda_Offer_Letter_Juan.docx"
doc.save(out)
print("saved:", out)