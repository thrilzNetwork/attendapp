...
CREATE POLICY default_insert ON public.weekly_forecasts FOR INSERT WITH CHECK (
  auth.role() = 'authenticated'
  AND hotel_id::text = (SELECT hotel_id::text FROM public.staff_accounts WHERE id = auth.uid() LIMIT 1)
);
CREATE POLICY default_update ON public.weekly_forecasts FOR UPDATE USING (
  auth.role() = 'authenticated'
  AND hotel_id::text = (SELECT hotel_id::text FROM public.staff_accounts WHERE id = auth.uid() LIMIT 1)
);
