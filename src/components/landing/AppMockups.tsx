...
export function MessagesScreenMockup() {
  ...
  const TABS = [
    { id: 'orders',   label: 'Requests', icon: <ClipboardList size={9} /> },
    { id: 'messages', label: 'Messages', icon: <MessageSquare size={9} /> },
    { id: 'shuttle',  label: 'Shuttle',  icon: <Bus size={9} /> },
    { id: 'schedule', label: 'Schedule', icon: <Calendar size={9} /> },
  ];
  ...
  return (
    <div className="flex flex-col h-full bg-[#F9FAFB] overflow-hidden">
      <MobileTopBar activeTab="messages" tabs={TABS} />
      ...
      <div className="bg-white border-b border-gray-100 px-3 py-2.5 flex items-center gap-2">
        ...
        <div>
          <p className="text-[10px] font-black text-gray-900">James K. · Room 214</p>
          <p className="text-[8px] text-gray-400">Check-in Jun 15 · Check-out Jun 18</p>
        </div>
        <div className="ml-auto">
          <Badge color="#16A34A">Validated</Badge>
        </div>
      </div>

      {/* Messages */}
      <div className="flex-1 overflow-hidden px-3 py-2 space-y-1.5">
        {THREAD_MSGS.slice(0, step).map((m, i) => (
          <div
            key={i}
          ...
          >
            <div
              ...
              style={
                m.from === 'staff'
                  ? { backgroundColor: TEAL, color: 'white', borderBottomRightRadius: '4px' }
                  : { backgroundColor: 'white', color: '#374151', border: '1px solid #F3F4F6', borderBottomLeftRadius: '4px' }
              }
            >
              {m.text}
              <div className="text-[7px] mt-0.5 opacity-60 text-right">{m.time}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="shrink-0 bg-white border-t border-gray-200 px-3 py-2">
        <div className="flex gap-2 items-center">
          <div className="flex-1 bg-gray-50 rounded-full px-3 py-2 text-[9px] text-gray-400 border border-gray-200">
            Reply to James…
          </div>
          <button className="w-7 h-7 rounded-full flex items-center justify-center text-white" style={{ backgroundColor: TEAL }}>
            <Send size={11} />
          </button>
        </div>
      </div>
